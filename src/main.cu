...
//                                 const float *W, const int w0, const int w1, const int w2, const int w3, 
//                                 float *Y, const int y0, const int y1, const int y2, const int y3){
//   int t = blockIdx.x * 1024.0 + threadIdx.x;
//   int h, w;
//   int H_out = x1-w0+1;
//   int W_out = x2-w0+1;
...
__global__ void conv_forward_valid_unrolled_Y_kernel_1(const float *X, const int x0, const int x1, const int x2, const int x3, 
                                const float *W, const int w0, const int w1, const int w2, const int w3, 
                                float *Y, const int y0, const int y1, const int y2, const int y3){
  int t = threadIdx.x;
  int old_col = t;
  int old_row, new_row = t, new_col;
  for(old_row = 0; old_row < 32; old_row++){
    new_col = old_row;
    Y[new_row*32+new_col] = X[old_row*576+old_col];
  }
}

// __global__ void conv_forward_valid_unrolled_Y_kernel_2(const float *X, const int x0, const int x1, const int x2, const int x3, 
//                                 const float *W, const int w0, const int w1, const int w2, const int w3, 
//                                 float *Y, const int y0, const int y1, const int y2, const int y3){
//   int t = threadIdx.x;
//   int old_col = t;
...
// void unroll_host(const float *X, const int x0, const int x1, const int x2, const int x3, 
//                                const float *W, const int w0, const int w1, const int w2, const int w3, 
//                                float *Y, const int y0, const int y1, const int y2, const int y3){
...
void forward_operation(float *x, float *conv1, float *conv2, float *fc1,
                       float *fc2, int *out,
             float *device_x, float *device_a, float *device_ax, float* device_aw, 
             float *device_am, float *device_b, float *device_c,
  ...
  std::cout << "\nDimensions: ";
  std::cout << "xdims: " << xdims[0] << " " << xdims[1] << " " << xdims[2] << " " << xdims[3] << "\n";
  std::cout << "conv1dims: " << conv1dims[0] << " " << conv1dims[1] << " " << conv1dims[2] << " " << conv1dims[3] << "\n";

  // unroll X
  int num_threads = xdims[3]*H_out_1*W_out_1;
  int num_blocks = ceil((float)num_threads/512.0);
  // dim3 DimGrid0X(adims[0], num_blocks, 1);
  dim3 DimGrid0X(num_blocks, 1, 1);
  dim3 DimBlock0X(512, 1, 1);
  ...
  // num_threads = H_out_1*W_out_1*conv1dims[3];
  // dim3 DimGrid0Y(ceil((float)num_threads/576.0), 1, 1);
  dim3 DimBlock0Y(576, 1, 1);
  ...
  // aw_test = allocate<float>(adims_w);
  // cudaMemcpy(aw_test, device_aw, adims_w[0]*adims_w[1]*sizeof(float), cudaMemcpyDeviceToHost);
  // std::cout << adims_w[0] << " " << adims_w[1];
  // std::cout << "device_aw: ";
  // for(int test = 0; test < adims_w[0]; test++){
  //   for(int test1 = 0; test1 < adims_w[1]; test1++){
  //     std::cout << aw_test[test*adims_w[1]+test1] << " ";
  //   }
  //   std::cout << "\n";
  // }
  ...
  // ax_test = allocate<float>(adims_x);
  // cudaMemcpy(ax_test, device_ax, adims_x[0]*adims_x[1]*sizeof(float), cudaMemcpyDeviceToHost);
  // std::cout << adims_x[0] << " " << adims_x[1];
  // std::cout << "device_ax: ";
  // for(int test = 0; test < adims_x[0]; test++){
  //   for(int test1 = 0; test1 < adims_x[1]; test1++){
  //     std::cout << ax_test[test*adims_x[1]+test1] << " ";
  //   }
  //   std::cout << "\n";
  // }

    // float* x_test;
    // x_test = allocate<float>(xdims);
    // cudaMemcpy(x_test, device_x, xdims[0]*xdims[1]*xdims[2]*xdims[3]*sizeof(float), cudaMemcpyDeviceToHost);
    // // std::cout << adims_x[0] << " " << adims_x[1];
    // std::cout << "\ndevice_x: ";
    
    // for(int test = 0; test < xdims[0]*xdims[1]*xdims[2]*xdims[3]; test++){
    //   if (test % 28 == 0) {
    //     std::cout << "\n";
    //   }
    //     // count_test++;
    //   // } 
    //   // else if(test!=0 && test%24 == 0){
    //   //   std::cout << "\n";
    //   // }
    //   //std::cout << std::setprecision(3) << am_test[test] << " ";
    //   printf("%c", (x_test[test] != 0) ? '#' : ' ');
    // }

  int count_test = 0;
  for(int n = 0; n < xdims[0]; n++){
    // cudaMemcpy(device_x, &x[n*xdims[1]*xdims[2]*xdims[3]], xdims[1]*xdims[2]*xdims[3]*sizeof(float), cudaMemcpyHostToDevice);
    conv_forward_valid_unrolled_X_kernel<<<DimGrid0X, DimBlock0X>>>(&device_x[n*28*28], xdims[0], xdims[1], xdims[2], xdims[3], 
                                                    device_conv1, conv1dims[0], conv1dims[1], conv1dims[2], conv1dims[3], 
                                                    device_ax, adims_x[0], adims_x[1]);
    // std::cout << &device_x[n*28*28] << "\n";
    // float* ax_test;
    // ax_test = allocate<float>(adims_x);
    // cudaMemcpy(ax_test, device_ax, adims_x[0]*adims_x[1]*sizeof(float), cudaMemcpyDeviceToHost);
    // std::cout << adims_x[0] << " " << adims_x[1];
    ...
    // for(int test = 0; test < adims_x[0]*adims_x[1]; test++){
    //   if (test % 576 == 0) {
    //     std::cout << "\n";
    //   }
    //     // count_test++;
    ...
    //   // }
    //   //std::cout << std::setprecision(3) << am_test[test] << " ";
    //   printf("%c", (ax_test[test] != 0) ? '#' : ' ');
    // }

    // float* x_test;
    // x_test = allocate<float>(xdims);
    // cudaMemcpy(x_test, &device_x[n*28*28], xdims[1]*xdims[2]*xdims[3]*sizeof(float), cudaMemcpyDeviceToHost);
    // // std::cout << adims_x[0] << " " << adims_x[1];
    // std::cout << "\ndevice_x: ";
    
    // for(int test = 0; test < xdims[1]*xdims[2]*xdims[3]; test++){
    //   if (test % 28 == 0) {
    //     std::cout << "\n";
    //   }
    //     // count_test++;
  ...
  // }
    ...
    //   }
    //   //std::cout << std::setprecision(3) << am_test[test] << " ";
    //   printf("%c", (am_test[test] != 0) ? '#' : ' ');
    // }
    // cudaDeviceSynchronize();
    conv_forward_valid_unrolled_Y_kernel<<<1, DimBlock0Y>>>(device_am, xdims[0], adims_m[0], adims_m[1], xdims[3], 
                                                                  device_conv1, conv1dims[0], conv1dims[1], conv1dims[2], conv1dims[3], 
                                                                  device_a+n*adims[1]*adims[2]*adims[3], adims[0], adims[1], adims[2], adims[3]);
  }
  cudaDeviceSynchronize();
  ...
  // std::cout << adims_m[0] << " " << adims_m[1];
  // std::cout << "device_am: ";
  // for(int test = 0; test < adims_m[0]; test++){
  //   for(int test1 = 0; test1 < adims_m[1]; test1++){
  //     std::cout << am_test[test*adims_m[1]+test1] << " ";
  //   }
  //   std::cout << "\n";
  // }
  ...
  // conv_forward_valid_unrolled_Y_kernel<<<DimGrid0Y, DimBlock0Y>>>(device_am, xdims[0], adims_m[0], adims_m[1], xdims[3], 
  //                                                                 device_conv1, conv1dims[0], conv1dims[1], conv1dims[2], conv1dims[3], 
  //                                                                 device_a, adims[0], adims[1], adims[2], adims[3]);
  // cudaDeviceSynchronize();

  // float* a_test;
  // a_test = allocate<float>(adims);
  // cudaMemcpy(a_test, device_a, adims[0]*adims[1]*adims[2]*adims[3]*sizeof(float), cudaMemcpyDeviceToHost);
  // std::cout << adims[0] << " " << adims[1] << " " << adims[2] << " " << adims[3];
  // std::cout << "device_a: ";
  // for(int test = 0; test < adims[0]*adims[1]*adims[2]*adims[3]; test++){
  //   std::cout << a_test[test] << " ";
  // }
...
int main(int argc, char **argv) {
  ...
  forward_operation(x, conv1, conv2, fc1, fc2, out,
          device_x, device_a, device_ax, device_aw, device_am, device_b, device_c, device_d, device_e, device_f, device_out,
